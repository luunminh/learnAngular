...
export class TaskService {
  ...
  onAddTask(newTask: Task) {
    ...
    this.onFilterTasks();
    this.snackbarService.onOpenSnackBar('Added a new task !!!', 'Close', 2000);
  }

  onEditTask(id: number, task: { title: string; description: string; status: Status }) {
    ...
    this.onFilterTasks();
    this.snackbarService.onOpenSnackBar('Updated your task !!!', 'Close', 2000);
  }

  onDeleteTask(id: number) {
    this.tasks = this.tasks.filter(task => task.id !== id);
    this.onFilterTasks();
    this.snackbarService.onOpenSnackBar('Deleted successful!', 'Close', 2000);
  }
